...
function showModal(modalId, button, sizeClass = "modal-md", isStatic = false) {
  ...
  const $userId = $(button).find('input[name="userId"]').val();
  const $username =
    $(button).find('input[name="firstName"]').val() +
    " " +
    $(button).find('input[name="lastName"]').val();
  const $gender = $(button).find('input[name="gender"]').val();
  const $email = $(button).find('input[name="email"]').val();
  const $photo =
  ...
  const photoPath = "/upload/users/" + $photo;
